...
SETTING_INFO: dict[str, SettingInfo] = {
    ...
    "amoc_temperature_coupling": _physical(
        "Fraction of the anomalous northern surface-to-deep temperature contrast allowed to affect the AMOC density driver.",
    ...
    "amoc_density_eos": _op(
        ...
        "Production default: teos10_matched with South Atlantic upper-water salinity and a transformed source temperature tied to northern surface-to-deep stratification. teos10_surface_watermass uses the local 35 S temperature as a structural sensitivity.",
    ...
    "initial_fovs": _physical(
        "Legacy SAU/deep internal-section target used to derive initial South Atlantic upper salinity. It is not the open-boundary FovS.",
        "Compatibility field; default -0.15 Sv. Excluded from built-in physical Monte Carlo priors.",
        "Historical CLEM configuration convention, retained for reproducibility.",
        "Low",
    ),
    ...
    "amoc_control_salinity_mode": _op(
        "Selects whether control salinity contrasts are solved from an explicit freshwater budget or read from the legacy hydrographic seeds.",
        "Default freshwater_budget. In that mode no FovS value or boundary salinity contrast enters the control solve.",
        "Model initialization choice.",
    ),
    "amoc_control_north_surface_freshwater_sv": _physical(
        "Steady-equivalent surface freshwater gain between 26.5 N and Bering Strait; positive values freshen the ocean.",
        "Default +0.37 Sv from observed freshwater divergence. The equilibrium initialization assumes zero storage; the source notes that finite-period divergence can include storage.",
        "McDonagh et al. (2015), doi:10.1175/JCLI-D-14-00519.1.",
        "Medium",
    ),
    "amoc_control_lower_atlantic_surface_freshwater_sv": _physical(
        "Net surface freshwater gain between 34.5 S and 26.5 N; negative values represent net evaporation.",
        "Default -0.65 Sv, the residual after treating northern +0.37 Sv divergence as a steady-equivalent surface term within the independently estimated basin-wide -0.28 Sv surface budget.",
        "Talley (2008), doi:10.1016/j.pocean.2008.05.001; McDonagh et al. (2015).",
        "Medium",
    ),
    "amoc_control_northern_boundary_freshwater_sv": _physical(
        "Non-overturning freshwater import through the northern Atlantic/Arctic boundary.",
        "Default +0.06 Sv for Bering Strait freshwater import.",
        "Talley (2008), doi:10.1016/j.pocean.2008.05.001.",
        "Medium",
    ),
    "amoc_control_southern_gyre_freshwater_sv": _physical(
        "Azonal circulation freshwater import across the southern Atlantic boundary.",
        "Default +0.38 Sv. This term is distinct from FovS and is needed because overturning alone does not close the Atlantic freshwater budget.",
        "Weijer et al. (1999), as summarized by de Vries and Weber (2005), doi:10.1029/2004GL021450.",
        "Medium",
    ),
    "initial_southern_salinity_psu": _physical(
        "Initial Southern Ocean surface-box salinity.",
        "Built-in prior support: 33.70–34.30 PSU; this box is not the revised default hydraulic source.",
...
ALIASES: dict[str, str] = {
    ...
    "amoc_control_north_surface_freshwater": "amoc_control_north_surface_freshwater_sv",
    "amoc_control_lower_atlantic_surface_freshwater": "amoc_control_lower_atlantic_surface_freshwater_sv",
    "amoc_control_northern_boundary_freshwater": "amoc_control_northern_boundary_freshwater_sv",
    "amoc_control_southern_gyre_freshwater": "amoc_control_southern_gyre_freshwater_sv",
