"""Release-finalization and evidence-integrity checks for CLEM v2.29.29."""
...
def test_v22929_identity_and_selected_defaults() -> None:
    cfg=ModelConfig()
...
def test_v22929_documents_are_synchronized() -> None:
    assert (ROOT/"README.md").read_text().startswith("# Coupled Low-complexity Earth Model v2.29.29")
    assert "## 2.29.29" in (ROOT/"CHANGELOG.md").read_text()
...
def test_v22929_retrospective_fold_local_manifest_is_semantically_honest() -> None:
...
def test_v22929_scientific_wording_matches_fail_closed_status() -> None:
...
def test_v22929_present_observational_sources_pass_hash_integrity() -> None:
