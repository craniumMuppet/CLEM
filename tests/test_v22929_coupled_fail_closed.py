"""Regression tests for the v2.29.29 coupled-validation trust boundary."""
...
from tools import finalize_v22929_release as finalizer
from tools.v22929_release_integrity import artifact_bundle_payload
...
def test_resolution_loader_rejects_failed_coupled_run(tmp_path: Path) -> None:
    ...
    _write(
        tmp_path / "SEA_ICE_VALIDATION_V2_29_29_5DEG.json",
        {**shared, "validation_passed": True},
    )
    _write(
        tmp_path / "ARCTIC_GREENLAND_AMOC_VALIDATION_V2_29_29_5DEG.json",
        {
            **shared,
...
def test_finalizer_rejects_two_failed_coupled_files_even_if_summary_claims_complete(
    ...
    for resolution in (5, 10):
        _write(
            tmp_path / f"SEA_ICE_VALIDATION_V2_29_29_{resolution}DEG.json",
            {
                **shared,
        ...
        (tmp_path / f"COUPLED_TIMESERIES_V2_29_29_{resolution}DEG.csv").write_text(
            "year,value\n2100,1\n", encoding="utf-8"
        )
    bundle_members = (
...
def test_current_coupled_entry_points_exist() -> None:
    ...
    assert (root / "validate_v22929.py").is_file()
    assert (root / "combine_v22929_validation.py").is_file()
    assert (root / "validate_v22929_coupled.py").is_file()
    wrapper = (root / "validate_v22929_coupled.py").read_text(encoding="utf-8")
