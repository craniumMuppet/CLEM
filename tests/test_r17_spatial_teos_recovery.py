...
def test_reference_support_uses_fixed_pack_boundary_and_obeys_extent_bounds():
    ...
    probe.config = cm.ModelConfig(arctic_ice_support_reference_mode="fixed_pack_80")
    s = probe._arctic_reference_ice_support_fraction(c)
...
def test_r17_runner_is_staged_bounded_and_reuses_one_collapse_seed():
    ...
    archived = Path(__file__).resolve().parents[1] / "provenance" / "r17-parent" / "verify_r17_local.py"
    spec = importlib.util.spec_from_file_location("archived_verify_r17_local", archived)
