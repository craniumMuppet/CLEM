#!/usr/bin/env python3
"""Combine v2.29.22 5-degree and 10-degree release-validation outputs."""
...
def resolution_payload(output_dir: Path, label: str) -> dict[str, Any]:
    sea_path = output_dir / f"SEA_ICE_VALIDATION_V2_29_22_{label}DEG.json"
    coupled_path = (
        output_dir / f"ARCTIC_GREENLAND_AMOC_VALIDATION_V2_29_22_{label}DEG.json"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional engineering-test JSON. Defaults to OUTPUT_DIR/TEST_RESULTS_V2_29_22.json, then the current directory.",
    )
    ...
    if test_results_path is None:
        output_candidate = args.output_dir / "TEST_RESULTS_V2_29_22.json"
        cwd_candidate = Path("TEST_RESULTS_V2_29_22.json")
    ...
    output_path = args.output_dir / "VALIDATION_SUMMARY_V2_29_22.json"
    ...
    if not engineering_release_passed:
        raise SystemExit("v2.29.22 engineering-release criteria are not all satisfied")
