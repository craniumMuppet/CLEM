#!/usr/bin/env python3
"""Lean, resumable scientific checks for EGCM v2.29.25.
...
def _historical_records(resolution: float) -> tuple[ModelConfig, pd.DataFrame, float]:
    ...
    fixed_area = {}
    for month in (3, 9):
        frame = load_homogeneous_area_month(month)
        fixed_area[month] = frame.set_index("year") if frame is not None else None
    ...
    def capture(step: int, elapsed: float) -> None:
        ...
        homogeneous = fixed_area[month]
        observed_fixed_area = (
            float(homogeneous.loc[year, "area"])
            if homogeneous is not None and year in homogeneous.index
            else float("nan")
        )
        rows.append({
            "year": year,
            ...
            "model_area": float(record["northern_hemisphere_sea_ice_thresholded_area_million_km2"]),
            "model_physical_area": float(record["northern_hemisphere_sea_ice_area_million_km2"]),
            ...
            "observed_raw_area_excluded": float(observed.loc[year, "area"]),
            "observed_area_is_homogeneous_fixed_mask": bool(
                homogeneous is not None and year in homogeneous.index
            ),
