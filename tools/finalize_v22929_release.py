#!/usr/bin/env python3
"""Verify and deterministically package the finalized CLEM v2.29.29 release."""
...
from v22929_release_integrity import (  # noqa: E402
    FINGERPRINT_JSON,
    MANIFEST,
    MODEL_VERSION,
    PACKAGE_INFO,
...
PROSPECTIVE_EVIDENCE_JSON = "validation/prospective/R16_PROSPECTIVE_EVIDENCE.json"
PROSPECTIVE_PROTOCOL_JSON = "validation/prospective/CLEM_R16_PROSPECTIVE_PROTOCOL.json"
...
def derived_arctic_status(tests: dict[str, Any]) -> dict[str, Any]:
    ...
    prospective_path = ROOT / PROSPECTIVE_EVIDENCE_JSON
    prospective = evaluate_r16_prospective(
        prospective_path if prospective_path.is_file() else None
    )
    ...
    prospective_passed = bool(
        prospective.get(
            ...
            prospective_complete and prospective_status == "passed",
        )
    )
    ...
    return {
        ...
        "independent_predictive_scientific_validation_status": prospective_status,
        "independent_predictive_scientific_validation_complete": prospective_complete,
        "independent_predictive_scientific_validation_passed": prospective_passed,
        ...
        # Backwards-compatible release-facing alias: this means the frozen
        # prospective evaluation has *passed*, not merely that it was computable.
        "scientific_predictive_validation_complete": prospective_passed,
    }
...
def write_package_info(tests: dict[str, Any], fingerprint: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "release_classification": (
            ...
            if status["scientific_predictive_validation_complete"]
            else (
                "engineering_release; independent_predictive_validation_failed"
                if status["independent_predictive_scientific_validation_status"] == "failed"
                else "engineering_release; independent_predictive_validation_not_available"
            )
        ),
        "test_evidence": {
            "file": TEST_JSON,
        ...
        "scientific_status_sources": [
            RECALIBRATION_JSON,
            RETROSPECTIVE_JSON,
            PROSPECTIVE_PROTOCOL_JSON,
            PROSPECTIVE_EVIDENCE_JSON if (ROOT / PROSPECTIVE_EVIDENCE_JSON).is_file() else None,
        ],
    }
    payload["scientific_status_sources"] = [
        item for item in payload["scientific_status_sources"] if item is not None
    ]
