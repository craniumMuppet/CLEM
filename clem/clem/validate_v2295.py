#!/usr/bin/env python3
"""Generate synchronized v2.29.5 accuracy and evidence-integrity validation records."""
...
def _future_sea_ice_metrics(result: Any) -> dict[str, Any]:
    ...
    for year in range(2021, 2101):
        ...
        raw_area = float(metrics["raw_two_sector_northern_ice_area_million_km2"])
        rows.append((year, area, extent, raw_area))
    ...
    return {
        ...
        "evidence_role": "native_process_projection_with_historical_observation_operator",
        "used_for_tuning": False,
        "independent_predictive_validation": False,
        "interpretation": (
            "Native prognostic thermodynamic ice is reported alongside a frozen "
            "1979-2020 hemispheric observation operator. No post-2020 trend, "
            "scenario closure, or future-target adjustment is applied. The "
            "longitude reconstruction remains display-only and has no regional forecast skill."
        ),
    }
...
def _arctic_reference_task() -> dict[str, Any]:
    ...
    for month in range(1, 13):
        ...
        monthly_native_area.append(
            raw_northern_ice_area_million_km2(
                state["atlantic_ice_fraction"],
                state["non_atlantic_ice_fraction"],
                model.grid.lat,
...
def _assemble_records(tasks: dict[str, Any]) -> None:
    ...
    summary = {
        ...
        "supplementary_pathways_not_used_for_v2295_integrity_fix_tuning": {
    ...
    release_checks = {
        "v2295_version": MODEL_VERSION == "2.29.5",
        ...
        "holdout_beats_frozen_persistence": bool(
            ssp245["sea_ice_validation"].get("all_holdout_metrics_beat_frozen_2020_persistence", False)
        ),
        "native_control_sea_ice_area": bool(
            12.0 <= native_reference["native_march_ice_area_million_km2"] <= 17.0
            and 3.0 <= native_reference["native_september_ice_area_million_km2"] <= 9.0
            and native_reference["native_september_ice_area_million_km2"]
        ...
        "nondegenerate_sea_ice_extent": bool(
            ssp245_future["september_extent_2081_2100_million_km2"] < 14.0
        ...
        "ssp585_late_century_native_sea_ice_area": bool(
            0.0 <= ssp585_future["native_september_area_2081_2100_million_km2"] <= 7.0
        ),
        "ssp245_late_century_observation_equivalent_sea_ice_area": bool(
            0.0 <= ssp245_future["september_area_2081_2100_million_km2"] <= 2.5
        ),
        "ssp585_late_century_observation_equivalent_sea_ice_area": bool(
            0.0 <= ssp585_future["september_area_2081_2100_million_km2"] <= 2.0
        ),
        ...
        "future_sea_ice_evidence_role": bool(
            all(
                item["evidence_role"] == "native_process_projection_with_historical_observation_operator"
        ...
        "continuous_control_balance": bool(abs(tasks["control"]["corrected_gmst_drift_c"]) <= 1.0e-10),
        ...
        "unconditional_monte_carlo_safety": bool(tasks["monte_carlo_safety"]["invalid_member_rejected"]),
        ...
        "hosing_recovery": bool(tasks["hosing_recovery"]["recovery_percent_of_initial_loss"] >= 80.0),
        "perturbation_recovery": bool(
            all(
                abs(item["final_arctic_air_c"]) <= 0.01 and item["final_amoc_sv"] >= 16.9
    ...
    write_json("VALIDATION_SUMMARY_V2_29_5.json", summary)
    write_json("DEEP_VALIDATION_V2_29_5.json", deep)
    ...
    audit = {
        ...
        "source_files": {
            ...
            for name in (
                ...
                "smoke_test.py", "validate_v2295.py",
        ...
        "implemented_structural_changes": {
            ...
            "holdout_skill_must_beat_frozen_2020_persistence": True,
            "exact_zero_native_ice_maps_to_zero_area_and_extent": True,
            "native_control_sea_ice_area_is_release_gated": True,
            "open_water_observational_plausibility_validation": True,
            "reproducible_noaa_oisst_processing_script_and_sources": True,
            ...
            "future_sea_ice_projection_uses_native_state_and_frozen_historical_operator": True,
        ...
        "known_scope_limits": [
            ...
            "Development benchmark ranges were used during tuning and are not independent validation; the separate NSIDC 2021-2025 temporal holdout is independent of the 1979-2020 observation-operator fit and is also compared with frozen-2020 persistence.",
    ...
    write_json("IMPLEMENTATION_AUDIT_V2_29_5.json", audit)
    print(json.dumps({
    ...
    if not deep["all_release_checks_passed"]:
        ...
        raise SystemExit("v2.29.5 release validation failed: " + ", ".join(failed))
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="validation_v2295_") as directory:
        root = Path(directory)
