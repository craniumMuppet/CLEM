#!/usr/bin/env python3
"""Run one tree-bound non-slow pytest invocation and retain raw v2.29.23 evidence."""
...
JSON_NAME = "TEST_RESULTS_V2_29_23.json"
TXT_NAME = "TEST_RESULTS_V2_29_23.txt"
...
def write_outputs(
    ...
    tree_mismatch = fingerprint_mismatches(tree_before, tree_after)
    tree_unchanged = not any(tree_mismatch[key] for key in tree_mismatch)
    passed = bool(
        pytest_exit_code == 0
    ...
    payload: dict[str, Any] = {
        ...
        "test_selection": (
            "single canonical release-tree-bound complete repository-defined "
            "non-slow suite"
        ),
    ...
    (output_dir / TXT_NAME).write_text(
        "\n".join(
            [
                "EGCM v2.29.23 tree-bound single-invocation non-slow test results",
...
def main() -> None:
    ...
    tree_before = compute_release_tree_fingerprint(package_root)
    ...
    tree_after = compute_release_tree_fingerprint(package_root)
    tree_changed = tree_before != tree_after
    runner_exit_code = pytest_exit_code if pytest_exit_code != 0 else (3 if tree_changed else 0)
    write_outputs(
        output_dir,
        plugin,
        pytest_exit_code,
