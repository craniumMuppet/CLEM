...
The v2.29.28 observational-development configuration uses `arctic_new_ice_local_thickness_m=0.22 m`, `arctic_full_cover_equivalent_thickness_m=3.70 m`, `arctic_ice_concentration_exponent=0.56`, `arctic_ice_nonsolar_heat_loss_wm2=47.8 W/m2`, `arctic_ice_area_formation_volume_sensitivity=11.5`, `arctic_ice_area_formation_support_floor=0.59`, `arctic_forced_ocean_heat_convergence_wm2_per_k=7.5 W/m2/K`, `arctic_forced_ocean_heat_convergence_onset_warming_c=0.45 C`, and `arctic_forced_ocean_heat_convergence_saturation_scale_c=0.32 C`. G02202 calibration now requires both <=1.0 M km2 fixed-mask RMSE and matching historical decline direction with model/observed trend magnitude between 2/3 and 1.5 in March and September. PIOMAS volume and the separate CryoSat-2/ICESat-2 thickness gates also pass. OSI SAF was inspected during method development and is therefore a non-independent cross-dataset diagnostic; its September mismatch is not a release gate. Scientific predictive validation remains incomplete: NSIDC-0611 is pending, retrospective fold-local prior-grid evaluation fails the required all-baseline skill condition, and untouched prospective validation is reserved for 2027 onward.

The release configuration limits `arctic_winter_transport_enhancement` to at most 25 W/m2/K and uses 10 W/m2/K by default. The seasonal operator requires both darkness and a sufficiently cold atmospheric state, so warm shoulder-season darkness does not receive deep-winter transport. Values above the release ceiling require explicit unsafe debug mode and are not scientific-release configurations.
