#!/usr/bin/env python3
"""Generate synchronized v2.29.15 accuracy and evidence-integrity validation records."""
...
def _winter_sea_ice_integrity_task() -> dict[str, Any]:
    ...
    for (
        ...
        sector_errors[prefix] = {
            "state_concentration_max_error": float(
                np.max(np.abs(state_concentration - reconstructed_concentration))
            ),
            "saved_local_thickness_max_error_m": float(
                np.max(np.abs(saved_local - reconstructed_local))
            ),
            "saved_open_water_temperature_max_error_c": float(
                np.max(np.abs(saved_open - reconstructed_open))
            ),
            "saved_volume_identity_max_error_m": float(
                np.max(
                    np.abs(
...
def _assemble_records(tasks: dict[str, Any]) -> None:
    """Assemble v2.29.15 records with evidence and release gates separated."""
    ...
    release_checks = {
        "v22915_version": MODEL_VERSION == "2.29.15",
    ...
    write_json("VALIDATION_SUMMARY_V2_29_15.json", summary)
    write_json("DEEP_VALIDATION_V2_29_15.json", deep)
    ...
    audit_source_names = (
        ...
        "validate_v22915.py",
        "tools/run_v22915_validation_parallel.py",
        "tools/reproduce_v22915_winter_review.py",
        ...
        "V2_29_15_WINTER_SEA_ICE_INTEGRITY.md", "V2_29_15_REVIEW_REPRODUCTION.json",
    ...
    write_json("IMPLEMENTATION_AUDIT_V2_29_15.json", audit)
    ...
    if not summary["all_release_checks_passed"]:
        ...
        raise SystemExit("v2.29.15 release validation failed: " + ", ".join(failed))
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="validation_v22915_") as directory:
