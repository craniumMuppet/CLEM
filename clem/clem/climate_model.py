...
MODEL_NAME = "Emergent-Sensitivity Global Climate Model"
...
class ModelConfig:
    ...
    deep_ocean_heat_capacity_wyr_m2_k: float = 110.0
    ...
    water_vapor_emission_height_km_per_lnq: float = 1.00
    ...
    # Five-box, salt-conserving Atlantic overturning model. Positive AMOC
    # follows Southern Ocean -> South Atlantic upper limb -> Tropical Atlantic
    # -> Northern sinking -> Deep return limb -> Southern Ocean. Negative
    # transport reverses that loop. The dedicated South Atlantic upper-limb
    # tracer is required for a physically meaningful FovS diagnostic at 34.5 S.
    ...
    # Smoothly saturate unresolved surface-to-deep thermal stratification at
    # very large anomalies, representing compensating deep-ocean warming and
    # mixing that a reduced box model cannot resolve explicitly.
    amoc_stratification_saturation_c: float = 0.60
    ...
    amoc_convection_temperature_density_coupling: float = 0.30
    ...
    amoc_interhemispheric_temperature_coupling: float = 0.02
    ...
    amoc_pycnocline_feedback_strength: float = 0.10
    ...
    # local density state sets a smooth convection target; active convection
    # entrains saline deep water into the sinking region, while weakening
    # convection removes that stabilising salt supply. This closes a continuous
    # convection-salinity-AMOC feedback loop without any Boolean "collapse now"
    # command. Collapse is diagnosed from the resulting transport only.
    amoc_convection_critical_density_ratio: float = 0.91
    amoc_convection_transition_width: float = 0.035
    amoc_convection_density_scale_factor: float = 4.00
    ...
    amoc_reference_density_driver: float = 7.5e-4
    ...
    initial_southern_salinity_psu: float = 34.55
    ...
    amoc_south_atlantic_external_exchange_sv: float = 2.0
    ...
    amoc_heat_response_damping_wm2_k: float = 1.35
    ...
    amoc_surface_heat_coupling_fraction: float = 0.075
    ...
    greenland_max_freshwater_sv: float = 0.025
    ...
    amoc_equilibrium_convection_max: float = 1.05
...
def _baseline_amoc_reference_temperatures(
    ...
    analytic_ocean = 25.5 - 34.0 * np.sin(np.deg2rad(grid.lat)) ** 2
    baseline_ocean_c = _zonal_surface_means(
    ...
    north = weighted_mean(
        baseline_ocean_c,
        grid.band_area_weights * grid.ocean_fraction * north_fraction,
    )
    southern = weighted_mean(
        baseline_ocean_c,
        grid.band_area_weights * grid.ocean_fraction * southern_fraction,
    )
...
def initial_amoc_density_diagnostics(
    ...
    *,
    ...
    thermal = float(config.thermal_expansion_per_k) * (southern - north)
    haline = float(config.haline_contraction_per_psu) * (
        float(config.initial_north_salinity_psu)
        - float(config.initial_southern_salinity_psu)
    )
    driver = float(thermal + haline)
    ratio = float(driver / float(config.amoc_reference_density_driver))
    ...
    *,
    ...
    if config.amoc_enforce_initial_density_constraint and not (
        ...
        raise ValueError(
            ...
            f"range: ratio={diagnostics['density_ratio']:.4f}, "
...
def saturation_specific_humidity(
    ...
    exponent = np.clip(17.67 * temperature_c / (temperature_c + 243.5), -80.0, 80.0)
    vapor_pressure = 611.2 * np.exp(exponent)
...
class ProcessClimateModel:
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.baseline_amoc_north_c = native_amoc_ocean_mean(
            self.amoc_north_region_fraction
        )
        self.baseline_amoc_southern_c = native_amoc_ocean_mean(
        ...
        def branch_step(
            *,
            ...
            if self._arctic_process_ledger_enabled:
                ...
                actual_area_candidate = actual_area_result
                reference_area_candidate = reference_area_result
                actual_area_process = None
                reference_area_process = None
            ...
            if self._arctic_process_ledger_enabled:
                assert actual_area_process is not None
                assert reference_area_process is not None
                area_process_names = (
    ...
    def _unresolved_polar_lapse_rate_feedback(
        ...
        return (
            cfg.arctic_lapse_rate_feedback_wm2_k * arctic_shape
    ...
    def radiative_fluxes(
        ...
        def ocean_components(
            ...
            components = {
                ...
                + self._unresolved_polar_lapse_rate_feedback(anomaly),
        ...
        land = {
            ...
            + self._unresolved_polar_lapse_rate_feedback(state.land_anomaly_c),
    ...
    def _density_driver_from_values(
        ...
        cfg = self.config
        # The prognostic Atlantic temperature response now contributes to the
        # density driver. A limited coupling coefficient is retained because a
        # reduced-complexity box model cannot resolve all atmospheric and ocean
        # compensation processes. Half of the coupling represents the changing
        # interhemispheric surface contrast and half represents northern
        # upper-ocean stratification relative to the deep reservoir. The two
        # pathways have separate explicit coupling coefficients.
        baseline_delta_t = self.baseline_amoc_southern_c - self.baseline_amoc_north_c
        north_anomaly = north_temperature_c - self.baseline_amoc_north_c
        southern_anomaly = southern_temperature_c - self.baseline_amoc_southern_c
        interhemispheric_anomaly = southern_anomaly - north_anomaly
        northern_stratification_anomaly = (
            north_surface_anomaly_c - north_deep_anomaly_c
        )
        saturation = cfg.amoc_stratification_saturation_c
        effective_stratification_anomaly = float(
            saturation
            * math.tanh(northern_stratification_anomaly / saturation)
        )
        coupling = cfg.amoc_temperature_density_coupling
        effective_delta_t = (
            baseline_delta_t
            + cfg.amoc_interhemispheric_temperature_coupling * interhemispheric_anomaly
            - coupling * effective_stratification_anomaly
        )
        thermal = cfg.thermal_expansion_per_k * effective_delta_t
        haline = cfg.haline_contraction_per_psu * (
            north_salinity_psu - southern_salinity_psu
        )
    ...
    def _project_salinity_to_conserved_total(
        self, salinity_psu: np.ndarray
    ) -> np.ndarray:
        """Project only floating-point salt roundoff onto the external box.

        The pre-projection residual is retained as an independent diagnostic.
        A residual larger than the configured roundoff ceiling raises instead
        of being silently concealed by exact projection.
        """
        salinity = np.asarray(salinity_psu, dtype=float).copy()
        residual = self.initial_total_salt_psu_m3 - float(
            np.sum(self.amoc_box_volumes_m3 * salinity)
        )
        residual_ppm = 1.0e6 * residual / self.initial_total_salt_psu_m3
        self._maximum_pre_projection_salt_error_ppm = max(
            self._maximum_pre_projection_salt_error_ppm,
            abs(float(residual_ppm)),
        )
        ...
        for _ in range(2):
            correction = self.initial_total_salt_psu_m3 - float(
                np.sum(self.amoc_box_volumes_m3 * salinity)
            )
    ...
    def _advective_mixing_salinity_tendency(
        ...
        Ocean, deep return limb, external reservoir. Positive overturning
        follows SO -> SAU -> T -> N -> D -> SO. Negative overturning reverses
        the loop; no absolute value is used, so reversed circulation transports
        ...
        q = abs(float(amoc_sv))
        if amoc_sv >= 0.0:
            upstream = {n: t, t: sau, sau: so, d: n, so: d}
        else:
            upstream = {n: d, d: so, so: sau, sau: t, t: n}
    ...
    def _greenland_surface_mass_balance(
        ...
        surface_temperature = reference_temperature + anomaly
        ...
        precipitation_multiplier = max(
            0.20,
            1.0 + cfg.greenland_precipitation_fraction_per_k * anomaly,
        )
    ...
    def _amoc_diagnostics(self, state: ModelState) -> dict[str, float]:
        ...
        # Scale departures from the exact grid-specific control state by one
        # fixed dimensional density normalizer. Dividing by each grid's own
        # baseline gradient made the forced AMOC response resolution-dependent.
        density_ratio = float(
            1.0
            + (driver - self.baseline_density_driver)
            / cfg.amoc_reference_density_driver
        )
        ...
        # branch. At the default 0.10, a 4% raw depth multiplier becomes a 0.4%
        ...
        # Deep-convection efficiency decreases smoothly as local surface water
        ...
        convection_multiplier = max(state.convection_efficiency, 0.0) ** (
        ...
        salinity = self._salinity_array(state)
        total_salt = float(np.sum(self.amoc_box_volumes_m3 * salinity))
        ...
        return {
            ...
            "amoc_temperature_density_term": cfg.thermal_expansion_per_k * (
                (self.baseline_amoc_southern_c - self.baseline_amoc_north_c)
                + cfg.amoc_interhemispheric_temperature_coupling
                * (
                    (temperatures["southern"] - self.baseline_amoc_southern_c)
                    - (temperatures["north"] - self.baseline_amoc_north_c)
                )
            ...
            "amoc_salinity_density_term": cfg.haline_contraction_per_psu
    ...
    def _coupled_amoc_rates(
        ...
        pycnocline_rate = (
            ...
            * SECONDS_PER_YEAR
            ...
            / cfg.amoc_pycnocline_relaxation_years
        )
        return {
    ...
    def _apply_reference_step_residual(
        ...
        corrected.convection_efficiency = float(
            np.clip(corrected.convection_efficiency, 0.0, 1.05)
        )
        ...
        if self.config.seasonal_arctic_enabled:
            reference = self._arctic_reference_state(elapsed_years)
            for prefix, energy_field, concentration_field, seasonal_field, public_field in (
                (
                    "atlantic",
    ...
    def _step_uncorrected(self, elapsed_years: float, dt_years: float | None = None) -> None:
        ...
        land_new = land_provisional + transport_delta
        atlantic_new = atlantic_provisional + transport_delta
        non_atlantic_new = non_atlantic_provisional + transport_delta
        ...
        if cfg.amoc_coupling_scheme == "euler":
            ...
            convection_new = float(np.clip(convection_new, 0.0, 1.05))
        ...
        salinity_new = self._project_salinity_to_conserved_total(salinity_new)
    ...
    def _check_state(self) -> None:
        ...
        if not 0.0 <= self.state.convection_efficiency <= 1.05:
...
def _amoc_equilibrium_tendency(
    ...
    pycnocline_tendency = (
        ...
        * SECONDS_PER_YEAR
        ...
        + (
            model.config.amoc_initial_pycnocline_depth_m
            - state.pycnocline_depth_m
        )
        / model.config.amoc_pycnocline_relaxation_years
    )
    return np.array(
        [
            *salinity_tendency[:5],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run a latitude-band process climate model in which ECS is diagnosed, "
            "not prescribed."
        ),
    ...
    parser.add_argument("--water-vapor-height", type=float, default=1.00, help="Emission-height increase in km per ln(q/q0)")
    ...
    parser.add_argument("--greenland-max-freshwater-sv", type=float, default=0.025)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Coupling of northern upper-ocean stratification to the AMOC density "
            "driver. Larger values make surface warming relative to the deep ocean "
            "weaken overturning more strongly."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="Coupling of the Atlantic north-south surface-temperature contrast to AMOC density.",
    )
    parser.add_argument(
        ...
        help=(
            "Smooth saturation scale for unresolved northern surface-to-deep "
            "thermal stratification in the hydraulic AMOC response."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=150.0,
        help="Relaxation time of pycnocline depth toward its control value.",
    )
    parser.add_argument(
        "--amoc-convection-critical-density-ratio",
        type=float,
        default=0.91,
        help="Local northern surface-to-deep density ratio around which convection efficiency declines continuously.",
    )
    parser.add_argument(
        "--amoc-convection-transition-width",
        type=float,
        default=0.035,
        help="Width of the smooth deep-convection transition in density-ratio units.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Exponent coupling convection efficiency directly to AMOC transport.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--amoc-equilibrium-convection-max", type=float, default=1.05)
    ...
    parser.add_argument("--amoc-reference-density-driver", type=float, default=7.5e-4)
