#!/usr/bin/env python3
"""Create canonical relocation-safe v2.29.23 engineering-only status files."""
...
CANONICAL_STATUS = "PACKAGE_STATUS_V2_29_23.json"
COMPATIBILITY_STATUS = "REVIEW_CORRECTED_STATUS.json"
TEST_JSON = "TEST_RESULTS_V2_29_23.json"
SUMMARY_JSON = "VALIDATION_SUMMARY_V2_29_23.json"
...
def verify_raw_test_evidence(root: Path, tests: dict[str, Any]) -> None:
    ...
    events_name = "TEST_EVENTS_V2_29_23.ndjson"
    junit_name = "TEST_RESULTS_V2_29_23.junit.xml"
...
def verify_primary_documentation(root: Path) -> None:
    readme = (root / "README.md").read_text(encoding="utf-8")
    changelog = (root / "CHANGELOG.md").read_text(encoding="utf-8")
    settings = (root / "SETTING_REFERENCE_GUIDE.md").read_text(encoding="utf-8")
    scientific_constraints = (root / "SCIENTIFIC_CONSTRAINTS.md").read_text(
        encoding="utf-8"
    )
    required_readme = (
        "# Emergent-Sensitivity Global Climate Model v2.29.23",
        "**Current classification:** v2.29.23",
        "run_v22923_engineering_tests.py",
        "validate_v22923.py",
        "combine_v22923_validation.py",
        "TEST_RESULTS_V2_29_23.json",
        "VALIDATION_SUMMARY_V2_29_23.json",
    )
    missing = [item for item in required_readme if item not in readme]
    if missing:
        raise SystemExit(f"README v2.29.23 semantics are incomplete: {missing}")
    if not changelog.startswith("# Changelog\n\n## 2.29.23\n"):
        raise SystemExit("CHANGELOG.md does not begin with v2.29.23")
    if not any(
        line.startswith("Version 2.29.23") for line in settings.splitlines()[:5]
    ):
        raise SystemExit("SETTING_REFERENCE_GUIDE.md is not framed as v2.29.23")
    if not scientific_constraints.startswith(
        "# Scientific constraints used by v2.29.23\n"
    ):
        raise SystemExit("SCIENTIFIC_CONSTRAINTS.md is not framed as v2.29.23")
...
def main() -> None:
    ...
    if tests.get("tree_unchanged_during_pytest") is not True:
        raise SystemExit("Engineering tests were not completed on one unchanged tree")
    tree_fingerprint = tests.get("release_tree_fingerprint")
    if not isinstance(tree_fingerprint, dict):
        raise SystemExit("Test evidence lacks the canonical release-tree fingerprint")
    verify_release_tree_fingerprint(root, tree_fingerprint)
    verify_primary_documentation(root)
    ...
    for label in ("5", "10"):
        ...
        resolution_status[f"{label}_degree"] = {
            ...
            "sea_ice_file": f"SEA_ICE_VALIDATION_V2_29_23_{label}DEG.json",
            "coupled_file": f"ARCTIC_GREENLAND_AMOC_VALIDATION_V2_29_23_{label}DEG.json",
            "timeseries_file": f"COUPLED_TIMESERIES_V2_29_23_{label}DEG.csv",
        }
    ...
    payload: dict[str, Any] = {
        ...
        "complete_non_slow_test_suite": {
            ...
            "release_tree_fingerprint_verified": True,
            "release_tree_sha256": tree_fingerprint["aggregate_sha256"],
            "release_tree_file_count": tree_fingerprint["file_count"],
        ...
        "source_corrections": {
            ...
            "production_process_ledger": "actual_receiving_reservoir_closure_and_implementation_mutation_tested",
            "relocation_safe_scientific_fingerprint": "implemented_19_relative_files",
            "canonical_release_tree_test_binding": "implemented_and_reproduced",
            "self_contained_single_invocation_test_evidence": "implemented",
            "packaging_compile_import_gui_smoke": "implemented",
            ...
            "primary_documentation": "updated_to_v2.29.23",
        },
        "source_hashes": source_hashes,
        "release_tree_fingerprint": tree_fingerprint,
