#!/usr/bin/env python3
"""Run all v2.29.16 validation tasks in isolated parallel subprocesses.
...
from validation_provenance import validate_task_record

VALIDATOR = ROOT / "validate_v22916.py"
TASK_NAMES = (
    "summary_ssp245",
    "summary_ssp585",
    "summary_ssp126",
...
def load_task_output(path: Path, task_name: str) -> dict:
    record = json.loads(path.read_text(encoding="utf-8"))
    value = validate_task_record(
        record,
        root=ROOT,
    ...
    if not isinstance(value, dict):
        raise ValueError(f"Validation task result is not an object: {path}")
    return value
...
def run_task(name: str, output_dir: Path, timeout: float) -> tuple[str, dict]:
    ...
    return name, load_task_output(output, name)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "validation_v22916_tasks_final",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    for name in TASK_NAMES:
        ...
        if args.resume and output.is_file():
            try:
                tasks[name] = load_task_output(output, name)
    ...
    from validate_v22916 import _assemble_records
