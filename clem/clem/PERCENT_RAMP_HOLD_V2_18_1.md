...
The rate list is the only user-facing growth-rate control. One value requests a
single pathway; multiple comma-separated values request a comparison. The
desktop GUI exposes the same values under **CO2 pathway**. The preset
...
For Monte Carlo experiments, supply exactly one rate. Multiple rates represent
separate ensembles and are therefore rejected rather than silently selecting
one.
