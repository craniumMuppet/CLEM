...
AmocDensityEOS = Literal["linear", "teos10"]
...
class ModelConfig:
    ...
    # Linear alpha/beta remains the default until the local structural run has
    # quantified the TEOS-10 branch. Selecting teos10 requires the standard
    # ``gsw`` Python package and changes only the reduced box-density operator.
    ...
    def validate(self) -> None:
        ...
        if self.amoc_density_eos not in {"linear", "teos10"}:
            raise ValueError("amoc_density_eos must be linear or teos10")
...
def initial_amoc_density_diagnostics(
    ...
    *,
    ...
    if config.amoc_density_eos == "teos10":
...
class ProcessClimateModel:
    ...
    def _density_driver_from_values(
        ...
        if cfg.amoc_density_eos == "teos10":
            from amoc_density_r16 import teos10_density_driver
    ...
    def _amoc_diagnostics(self, state: ModelState) -> dict[str, float]:
        ...
        return {
            ...
            "amoc_density_eos_is_teos10": float(cfg.amoc_density_eos == "teos10"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--amoc-density-eos", choices=["linear", "teos10"],
        default=ModelConfig().amoc_density_eos,
        help="Reduced AMOC box equation of state; teos10 requires the optional gsw package.",
    )
