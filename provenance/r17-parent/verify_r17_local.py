#!/usr/bin/env python3
"""CLEM R17 staged local physics-validation runner.
...
OUT = ROOT / "r17_validation_results"
CHECKPOINTS = OUT / "_checkpoints"
BUNDLE = ROOT / "CLEM_v2.29.28_R17_validation_results.zip"
RUN_LOCK = ROOT / ".r17_validation_active.json"
MAX_CHUNK_YEARS = 5.0
DEFAULT_CHUNK_TIMEOUT_SECONDS = 300
DEFAULT_SETUP_TIMEOUT_SECONDS = 300
RECORD_INTERVAL_YEARS = 1.0
EPS = 1.0e-9
VERIFIER_REVISION = "2026-08-31-r17-spatial-teos-recovery"
R16_2_PARENT_SOURCE_ZIP_SHA256 = "80176acff4e2f7b5a50c7365ffeda16c807791a60de7a8016ae6914ed31d9bb6"
R16_2_PARENT_CLIMATE_MODEL_SHA256 = "08662fef5d83154d8bc43420705a937c6acbc4b52b4677a6e20536d73193506e"
R16_2_PARENT_SEA_ICE_OBSERVATION_SHA256 = "7d346f90b87d4940a379c1afd50cefaa8184bd3ea6a91fb10fef4e8406774a65"
R17_CLIMATE_MODEL_SHA256 = "7e80b668de0f30d278ec2b79de6438c4d0a89b546eca3585d6efb37132e2e450"
R17_SEA_ICE_OBSERVATION_SHA256 = "ae630ba91d8eaf194c892b0a83c1b5286c354355c260daff47e7053d80f63d95"
...
SEGMENTS: dict[str, dict[str, Any]] = {
    # Sea-ice: validate the new prognostic native-band support state at both
    # coupled resolutions. These are the only runs needed to judge the R17
    # extent/spatial-state change.
    "r17_seaice_eval_1850_2025_10deg": {"stage": "sea-ice", "duration": 175.0, "record_interval_years": 1.0 / 12.0, "config": {"start_year": 1850.0, "resolution_deg": 10.0, "scenario": "ssp245", "dt_years": 0.05, "seasonal_arctic_enabled": True}},
    "r17_seaice_eval_1850_2025_5deg": {"stage": "sea-ice", "duration": 175.0, "record_interval_years": 1.0 / 12.0, "config": {"start_year": 1850.0, "resolution_deg": 5.0, "scenario": "ssp245", "dt_years": 0.05, "seasonal_arctic_enabled": True}},
    "r17_seaice_ssp245_1850_2100_10deg": {"stage": "sea-ice", "duration": 250.0, "record_interval_years": 1.0 / 12.0, "config": {"start_year": 1850.0, "resolution_deg": 10.0, "scenario": "ssp245", "dt_years": 0.05, "seasonal_arctic_enabled": True}},
    "r17_seaice_ssp245_1850_2100_5deg": {"stage": "sea-ice", "duration": 250.0, "record_interval_years": 1.0 / 12.0, "config": {"start_year": 1850.0, "resolution_deg": 5.0, "scenario": "ssp245", "dt_years": 0.05, "seasonal_arctic_enabled": True}},

    # TEOS: apples-to-apples nonlinear EOS sensitivity. The old direct
    # surface-watermass TEOS evidence is already preserved in R16.2 and is not
    # rerun.
    "r17_teos_matched_control_20y": {"stage": "teos", "duration": 20.0, "config": {"resolution_deg": 10.0, "scenario": "constant", "dt_years": 0.05, "seasonal_arctic_enabled": True, "amoc_density_eos": "teos10_matched"}},
    "r17_teos_matched_hosing_0p2_100y": {"stage": "teos", "duration": 100.0, "config": {"resolution_deg": 10.0, "scenario": "constant", "dt_years": 0.05, "seasonal_arctic_enabled": True, "freshwater_hosing_sv": 0.20, "freshwater_start_fraction": 0.0, "freshwater_ramp_years": 1.0, "amoc_density_eos": "teos10_matched"}},
    "r17_teos_matched_ssp245_1850_2100_10deg": {"stage": "teos", "duration": 250.0, "record_interval_years": 0.25, "config": {"start_year": 1850.0, "resolution_deg": 10.0, "scenario": "ssp245", "dt_years": 0.05, "seasonal_arctic_enabled": True, "amoc_density_eos": "teos10_matched"}},

    # Recovery/hysteresis: collapse once, then clone that exact checkpoint into
    # three de-hosing branches. No restart threshold or recovery parameter is
    # introduced into the model.
    "r17_recovery_collapse_seed_250y": {"stage": "recovery", "duration": 250.0, "config": {"resolution_deg": 10.0, "scenario": "constant", "dt_years": 0.05, "seasonal_arctic_enabled": False, "freshwater_hosing_sv": 0.80, "freshwater_start_fraction": 0.0, "freshwater_ramp_years": 1.0}},
    "r17_recovery_dehose_m0p05_to_700y": {"stage": "recovery", "duration": 700.0, "inherits_from": "r17_recovery_collapse_seed_250y", "config": {}, "stages": [{"start": 250.0, "end": 700.0, "overrides": {"freshwater_hosing_sv": -0.05}}]},
    "r17_recovery_dehose_m0p10_to_700y": {"stage": "recovery", "duration": 700.0, "inherits_from": "r17_recovery_collapse_seed_250y", "config": {}, "stages": [{"start": 250.0, "end": 700.0, "overrides": {"freshwater_hosing_sv": -0.10}}]},
    "r17_recovery_dehose_m0p20_to_700y": {"stage": "recovery", "duration": 700.0, "inherits_from": "r17_recovery_collapse_seed_250y", "config": {}, "stages": [{"start": 250.0, "end": 700.0, "overrides": {"freshwater_hosing_sv": -0.20}}]},
}
STAGE_SEGMENTS = {
    "sea-ice": [name for name, spec in SEGMENTS.items() if spec["stage"] == "sea-ice"],
    "teos": [name for name, spec in SEGMENTS.items() if spec["stage"] == "teos"],
...
def validate_r17_provenance(source_hash: str) -> dict[str, Any]:
    """Fail closed on exact R16.2 parent snapshots and frozen R17 hashes."""
    provenance_path = ROOT / "R17_PARENT_PROVENANCE.json"
    ...
    if not provenance_path.exists() or not parent_climate.exists() or not parent_seaice.exists():
        raise SystemExit("R17 parent provenance is incomplete.")
    provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
    checks = {
        "current_climate_model": source_hash == R17_CLIMATE_MODEL_SHA256,
        "current_sea_ice_observation": sha256(ROOT / "sea_ice_observation.py") == R17_SEA_ICE_OBSERVATION_SHA256,
        "parent_zip": provenance.get("r16_2_parent_source_zip_sha256") == R16_2_PARENT_SOURCE_ZIP_SHA256,
        "parent_climate": sha256(parent_climate) == R16_2_PARENT_CLIMATE_MODEL_SHA256,
        "parent_seaice": sha256(parent_seaice) == R16_2_PARENT_SEA_ICE_OBSERVATION_SHA256,
        "manifest_current_climate": provenance.get("r17_climate_model_sha256") == source_hash,
        "manifest_current_seaice": provenance.get("r17_sea_ice_observation_sha256") == sha256(ROOT / "sea_ice_observation.py"),
        "no_default_amoc_retune": provenance.get("default_amoc_physics_retuned") is False,
        "recovery_is_experiment_only": provenance.get("amoc_restart_or_recovery_trigger_added") is False,
    }
    failed = [name for name, ok in checks.items() if not ok]
    if failed:
        raise SystemExit("R17 provenance check failed: " + ", ".join(failed))
...
def static_worker() -> dict[str, Any]:
    ...
    support_reference = cm.ProcessClimateModel._arctic_reference_ice_support_fraction(
    ...
    return {
        "r17_support_state": {
            ...
            "reference_support": support_reference.tolist(),
            "pass_support_at_least_native_area": bool(
                np.all(support_reference >= support_concentration - 1.0e-12)
            ),
            "pass_support_not_below_15pct_implied_concentration": bool(
                np.all(support_reference <= support_upper + 1.0e-12)
            ),
            "pass_reference_pack_boundary_is_80pct": bool(
                abs(float(cm.PACK_ICE_CONCENTRATION_THRESHOLD) - 0.80) < 1.0e-12
            ),
        },
        "r17_validation_design": {
            ...
            "pass_recovery_branches_share_one_seed": bool(
                all(
                    SEGMENTS[name].get("inherits_from") == "r17_recovery_collapse_seed_250y"
                    for name in STAGE_SEGMENTS["recovery"]
                    if name != "r17_recovery_collapse_seed_250y"
                )
            ),
        },
        "control": {
...
def setup_segment_worker(name: str) -> dict[str, Any]:
    ...
    if existing.exists():
        ...
        if payload.get("source_sha256") != source_hash or payload.get("spec_sha256") != spec_hash:
            raise RuntimeError(
                ...
                "Run verify_r17_local.py --fresh once."
            )
        return {
            "segment": name,
            "resumed": True,
    ...
    initial_record = model.record(elapsed0)
...
def finalize_results(static_result: dict[str, Any], segment_status: dict[str, Any]) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "candidate": "CLEM v2.29.28 Physics Candidate R17",
    ...
    payload["interpretation"] = "R17 validates only changed/new physics: prognostic sea-ice support, matched-pathway TEOS, and de-hosing recovery branches. Accepted R16/R16.2 baseline evidence is not rerun unnecessarily."
    return payload
...
def build_bundle(manifest: dict[str, Any]) -> None:
    ...
    for extra_name in ("amoc_density_r16.py", "prospective_validation_r16.py", "SOURCE_FINGERPRINT.json", "R17_PARENT_PROVENANCE.json", "R17_IMPLEMENTATION_AND_LOCAL_VALIDATION.md"):
...
def acquire_run_lock() -> None:
    if RUN_LOCK.exists():
        ...
        if old_pid != os.getpid() and _pid_is_running(old_pid):
            raise SystemExit(
                f"Another R17 validation process is still active (PID {old_pid}). "
...
def parent_main(args: argparse.Namespace) -> None:
    ...
    provenance_info = validate_r17_provenance(source_hash)
    print("R17 provenance accepted: exact R16.2 parent snapshots and frozen R17 source hashes.", flush=True)
    ...
    manifest: dict[str, Any] = {
        ...
        "r17_parent_provenance": provenance_info,
        "segments": {},
        "static": {},
    }

    print("CLEM R17 staged physics validation", flush=True)
    ...
    manifest["run_mode"] = f"r17_{args.stage}"
    ...
    print("Upload CLEM_v2.29.28_R17_validation_results.zip back to ChatGPT.", flush=True)


def main() -> None:
    parser = argparse.ArgumentParser(description="CLEM R17 staged verification with <=5-year restartable chunks")
    parser.add_argument("--fresh", action="store_true", help="delete old verification checkpoints/results and restart from zero")
    parser.add_argument("--stage", choices=["all", "sea-ice", "teos", "recovery"], default="all", help="run all changed-physics experiments or one stage only")
