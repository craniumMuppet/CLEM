...
def march_extent_trend_robustness(records: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "extent_is_separate_prognostic_state": True,
        "extent_derived_from_native_concentration": False,
        ...
        "mandatory_limitation": (
            ...
            "does not resolve satellite-scale spatial concentration. The separate "
            "prognostic support geometry is still a coarse native-band 15% extent "
            "diagnostic, not independently validated prediction skill."
        ),
...
def evaluate_result(result: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "area_operator": {
            ...
            "extent_method": "prognostic_native_band_support_fractional_15pct_threshold",
            "extent_observation_operator_calibrated": False,
            "extent_is_separate_prognostic_state": True,
            "extent_derived_from_native_concentration": False,
            "extent_uses_raw_nsidc_area": False,
            "extent_from_native_prognostic_concentration": False,
            "extent_independently_prognostic_spatial_field": False,
            "extent_has_separate_prognostic_geometry_state": True,
            "extent_interpretation": (
                "Extent is diagnosed from a separate prognostic native-band ice-support geometry state while native concentration still determines ice area and latent volume. "
                "The support state is conservative with respect to native ice area, uses fractional 15% occupancy, and contains no fitted historical area-to-extent multiplier. "
                "It remains a coarse reduced-order spatial diagnostic rather than a satellite-resolution concentration field."
            ),
        },
