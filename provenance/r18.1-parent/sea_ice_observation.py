...
PACK_ICE_CONCENTRATION_THRESHOLD = 0.80
...
def reconstruct_concentration_and_occupancy(
    *,
    ...
    warming_c: float,
    calendar_year: float,
    atlantic_support_fraction: np.ndarray | None = None,
    non_atlantic_support_fraction: np.ndarray | None = None,
) -> tuple[np.ndarray, np.ndarray, dict[str, float | str]]:
    """Return native concentration and conservative fractional 15% occupancy.

    When R17 prognostic support fractions are supplied, extent is integrated
    directly from that independent geometry state. The native concentration
    still determines ice area exactly, so the support state cannot create or
    destroy ice mass/area. Without support inputs this function retains the R16
    monotone subgrid reconstruction for backward-compatible diagnostics.
    """
    ...
    support_is_prognostic = (
        atlantic_support_fraction is not None and non_atlantic_support_fraction is not None
    )
    if support_is_prognostic:
        atl_occ = np.clip(np.asarray(atlantic_support_fraction, dtype=float), 0.0, 1.0)
        non_occ = np.clip(np.asarray(non_atlantic_support_fraction, dtype=float), 0.0, 1.0)
        if atl_occ.shape != atlantic.shape or non_occ.shape != non_atlantic.shape:
            raise ValueError("prognostic sea-ice support must match latitude-band concentration")
        # An occupied fraction must be large enough to hold the native ice area
        # at <=100% SIC, but not so large that its implied pack concentration
        # drops below the 15% extent threshold.
        atl_upper = np.divide(
            atlantic, MINIMUM_EXTENT_CONCENTRATION,
            out=np.zeros_like(atlantic), where=atlantic > 0.0,
        )
        non_upper = np.divide(
            non_atlantic, MINIMUM_EXTENT_CONCENTRATION,
            out=np.zeros_like(non_atlantic), where=non_atlantic > 0.0,
        )
        atl_occ = np.where(
            atlantic > 0.0,
            np.clip(atl_occ, atlantic, np.clip(atl_upper, 0.0, 1.0)),
            0.0,
        )
        non_occ = np.where(
            non_atlantic > 0.0,
            np.clip(non_occ, non_atlantic, np.clip(non_upper, 0.0, 1.0)),
            0.0,
        )
        atl_thr_int = atlantic
        non_thr_int = non_atlantic
    else:
    ...
    metrics: dict[str, float | str] = {
        ...
        "extent_method": (
            "prognostic_subgrid_ice_support_15pct_threshold"
            if support_is_prognostic
            else "conservative_unfitted_meridional_subgrid_15pct_threshold"
        ),
        "extent_contains_observational_fit": 0.0,
        "extent_is_separate_prognostic_state": float(support_is_prognostic),
        "extent_derived_from_native_concentration": float(not support_is_prognostic),
        "extent_subgrid_fractional_occupancy": 1.0,
        "extent_reconstruction_coordinate": (
            "prognostic_native_band_support"
            if support_is_prognostic
            else "sin(latitude)_equal_area"
        ),
        "pack_ice_concentration_threshold": PACK_ICE_CONCENTRATION_THRESHOLD,
