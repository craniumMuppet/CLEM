#!/usr/bin/env python3
"""Version-matched v2.29.23 scientific and engineering validation.
...
def structural_area_volume_experiments(config: ModelConfig) -> dict[str, Any]:
    ...
    def run_case(
        *,
        ...
        model = model_type(experiment_config)
    ...
    class ReversedArcticOceanReceiver(ProcessClimateModel):
        def _apply_arctic_mixed_layer_internal_transfer(
            self,
            ocean_temperature_anomaly_c: np.ndarray,
            transfer_wyr_m2: np.ndarray,
    ...
    class MisroutedLowerLatitudeReceiver(ProcessClimateModel):
    ...
    mutation_models: dict[str, type[ProcessClimateModel]] = {
        "suppressed_arctic_ocean_receiver": SuppressedArcticOceanReceiver,
        "reversed_arctic_ocean_receiver": ReversedArcticOceanReceiver,
        "misrouted_lower_latitude_ocean_receiver": MisroutedLowerLatitudeReceiver,
    }
    mutation_checks: dict[str, bool] = {}
    mutation_residuals: dict[str, dict[str, float]] = {}
    for mutation_name, model_type in mutation_models.items():
        mutant_entries = run_case(
            elapsed_years=0.50,
            air_anomaly_c=-15.0,
            ocean_anomaly_c=-3.0,
            ice_energy_anomaly_m=0.50,
        ...
        mutation_checks[f"{mutation_name}_detected"] = not result["passed"]
        mutation_residuals[mutation_name] = {
            "maximum_energy_wyr_m2": result[
                "maximum_energy_closure_residual_wyr_m2"
            ],
            "surface_to_arctic_ocean_wyr_m2": result[
                "maximum_surface_to_arctic_ocean_residual_wyr_m2"
            ],
            "lower_latitude_spatial_wyr_m2": result[
    ...
    return {
        ...
        "process_budget_experiments": {
            "method": "production_ProcessClimateModel_step_actual_receiving_reservoir_ledger",
            "case_entry_counts": {
