...
def model_fixed_mask_area_extent(
    ...
    *,
    ...
    area = float(np.sum(np.where(selected, concentration * operator.cell_area_km2, 0.0)) / 1.0e6)
    extent = float(np.sum(np.where(selected, operator.cell_area_km2, 0.0)) / 1.0e6)
    return {
