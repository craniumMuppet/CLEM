#!/usr/bin/env python3
"""Generate synchronized v2.29.3 release-integrity validation records."""
...
def _assemble_records(tasks: dict[str, Any]) -> None:
    ...
    summary = {
        ...
        "supplementary_pathways_not_used_for_v2293_integrity_fix_tuning": {
    ...
    release_checks = {
        "v2293_version": MODEL_VERSION == "2.29.3",
    ...
    write_json("VALIDATION_SUMMARY_V2_29_3.json", summary)
    write_json("DEEP_VALIDATION_V2_29_3.json", deep)
    ...
    audit = {
        ...
        "source_files": {
            ...
            for name in (
                ...
                "validate_v2293.py", "tests/test_v2292_release_integrity.py",
                "tests/test_v2293_operational_safety.py", "tools/package_v2293.py",
            )
        },
    ...
    write_json("IMPLEMENTATION_AUDIT_V2_29_3.json", audit)
    print(json.dumps({
    ...
    if not deep["all_release_checks_passed"]:
        ...
        raise SystemExit("v2.29.3 release validation failed: " + ", ".join(failed))
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="validation_v2293_") as directory:
        root = Path(directory)
